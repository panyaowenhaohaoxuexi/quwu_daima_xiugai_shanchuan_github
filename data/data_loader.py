...
# --- 修改后的 TestDataset 类 ---
class _LegacyTestDataset(data.Dataset):
    """
...
class _LegacyMultiModalHazeDataset(data.Dataset):
    def __init__(
        self,
        hazy_visible_path,
        infrared_path,
        ...
        if self.use_sky_mask:
            ...
            if not self.sky_mask_path or not os.path.isdir(self.sky_mask_path):
                ...
                if self.require_sky_mask:
                    raise RuntimeError(msg)
                print(f"WARNING: {msg}. Missing masks will fall back to all-zero masks.")
            sample_names = self.image_list[:20]
            found = sum(1 for name in sample_names if self.find_sky_mask_path(name) is not None)
            print(f"[SkyMask] sample check: found {found} / {len(sample_names)}")
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # --- 数据增强与预处理 ---
            processed = self.aug_and_preprocess(hazy_vis, infrared, clear_vis, sky_mask)
...
class _LegacyMultiModalCLIPLoader(data.Dataset):
