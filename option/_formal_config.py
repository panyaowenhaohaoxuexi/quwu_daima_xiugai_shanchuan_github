...
class _RecordExplicitTrainingObjective(argparse.Action):
    """Retain that a training-objective CLI field was intentionally supplied."""
...
class _RecordExplicitFormalTraining(argparse.Action):
    """Store formal-training true while recording that the flag was present."""
...
def _add_loss_weight_argument(parser, name, default):
    parser.add_argument(
        f"--{name}", type=float, default=default, action=_RecordExplicitTrainingObjective,
    )
