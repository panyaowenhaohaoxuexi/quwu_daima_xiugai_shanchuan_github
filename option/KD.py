"""
这段Python代码是一个用于知识蒸馏训练学生模型（Student Model）的配置和初始化脚本。
它使用 argparse 库来定义和解析一系列深度学习训练参数，包括训练轮数、学习率、损失权重（如L1、SSIM、Cr以及知识蒸馏特有的FA损失权重w_loss_FA）和文件路径。
在解析参数后，它会自动检测PyTorch是否可以使用GPU（CUDA）并设置设备。
该脚本的核心功能是根据这些配置（默认数据集为Student）自动创建一套层级化的实验目录（例如 ./experiment/Student/THaze/），并在其中生成 saved_model 和 saved_data 文件夹。
最后，它会将所有最终确定的配置参数保存为一个JSON格式的 args.txt 文件，存放在新创建的模型目录中，以便记录和复现该次学生模型的训练设置。
"""
...
# 定义数据集名称（用于构建目录结构），这里默认为 'Student'
...
# 构建特定数据集的目录路径 (例如 ./experiment/Student)
dataset_dir = os.path.join(opt.exp_dir, opt.dataset)
# 构建特定模型的目录路径 (例如 ./experiment/Student/THaze)
...
if not os.path.exists(dataset_dir):
    os.mkdir(dataset_dir)  # 创建 ./experiment/Student
if not os.path.exists(model_dir):
    os.mkdir(model_dir)  # 创建 ./experiment/Student/THaze
    ...
    os.mkdir(opt.saved_model_dir)  # 创建 ./experiment/Student/THaze/saved_model
    os.mkdir(opt.saved_data_dir)  # 创建 ./experiment/Student/THaze/saved_data
